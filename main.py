...
class Login(Handler):
    ...
    def post(self):
        submitted_username = self.request('username')
        submitted_password = self.request('password')
...
class Register(Handler):
    ...
    def validate_password(self, password):
        PWD_RE = re.compile(r"^.{3,20}$s")
        if PWD_RE.match(passowrd):
            return password
        return ""
    ...
    def post(self):
        ...
        if has_error:
            t=jinja_env.template("register.html")
